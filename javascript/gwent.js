...
class Player {
	...
	async activateFactionAbility() {
		...
		if (factionData.activeAbility && this.factionAbilityUses > 0) await ui.popup("Способность фракции [E]", () => this.useFactionAbility(), "Отмена [Q]", () => this.escapeFactionAbility(), "Хотите использовать способность своей фракции?", "Способность фракции: " + factionData.description, false);
		return;
	}
...
class Game {
	...
	async initialRedraw() {
		...
		if (player_me.controller instanceof ControllerAI) {
			...
			await ui.queueCarousel(player_me.hand, 2, async (c, i) => await player_me.deck.swap(c, c.removeCard(i)), c => true, true, true, "Выберите до 2 карт для повторного вытягивания.");
...
class Card {
	constructor(key, card_data, player) {
		...
		} else if (this.row === "agile") this.desc_name = "Гибкость";
		else if (this.hero) this.desc_name = "Герой";
		else this.desc_name = "";
		this.desc = this.row === "agile" ? "<p><b>Гибкость:</b> " + ability_dict["agile"].description + "</p>" : "";
		...
		if (this.abilities.includes("avenger") && this.target) {
			...
			this.desc += "<p>Призывает <b>" + target["name"] + "</b> с силой " + target["strength"];
			if (target["ability"].length > 0) this.desc += " и способности " + target["ability"].split(" ").map(a => ability_dict[a]["name"]).join(" / ");
			this.desc += "</p>";
		}
		if (this.hero) this.desc += "<p><b>Герой:</b> " + ability_dict["hero"].description + "</p>";
...
class UI {
	...
	async notification(name, duration) {
		var guia1 = {
			"notif-nilfgaard-wins-draws": "Нильфгаард выигрывает при ничьей",
			"notif-op-white-flame": "Лидер противника отменяет способность лидера противника.",
			"notif-op-leader": "Противник использует способность лидера",
			"notif-me-first": "Вы пойдете первым.",
			"notif-op-first": "Ваш противник сделает первый ход.",
			"notif-me-coin": "Вы пойдете первым.",
			"notif-op-coin": "Ваш противник сделает первый ход.",
			"notif-round-start": "Раунд начат",
			"notif-me-pass": "Раунд окончен",
			"notif-op-pass": "Ваш противник сделал пас",
			"notif-win-round": "Вы выиграли раунд!",
			"notif-lose-round": "Ваш противник выиграл раунд",
			"notif-draw-round": "Раунд завершился ничьей",
			"notif-me-turn": "Твоя очередь!",
			"notif-op-turn": "Ход противника",
			"notif-north": "Сработала способность фракции Северных Королевств — Север берет дополнительную карту.",
			"notif-monsters": "Сработала способность фракции Монстров — одна случайно выбранная карта юнита Монстра остаётся на игровом поле.",
			"notif-scoiatael": "Противник использовал преимущество фракции Скоя'таэль, чтобы начать ход первым.",
			"notif-skellige-op": "Способность противника Скеллиге сработала - противник забрал 2 случайные карты из биты!",
			"notif-skellige-me": "Способность Скеллиге активирована - вы получили 2 случайные карты из биты!",
			"notif-witcher_universe": "Способность Школы Ведьмаков использована - пропуск хода",
			"notif-toussaint": "Срабатывает способность фракции Туссент — Туссент берет дополнительную карту.",
			"notif-toussaint-decoy-cancelled": "Способность Вождя Туссена использована — способность Чучела отменена до конца раунда.",
			"notif-lyria_rivia": "Способность Лирии и Ривии использована — эффект повышения морали применяется к ряду.",
			"notif-meve_white_queen": "Лидер Лирии и Ривии позволяет обоим игрокам восстановать по 2 юнита при использовании способности медика.",
			"notif-north-scorch-cancelled": "Способность лидера Северных Королевств использована — способность «Сожжение» отменена до конца раунда.",
			"notif-zerrikania": "Способность Зеррикании использована — Юнит восстановлен из стопки сброса.",
		}
		var guia2 = {
			"me-pass" : "pass",
			"win-round" : "round_win",
...
class DeckMaker {
	...
	startNewGame(fullAI = false) {
		...
		openFullscreen();
		let warning = "";
		if (this.stats.units < 22) warning += "В вашей колоде должно быть не менее 22 карт юнитов. \n";
		if (this.stats.special > 10) warning += "В вашей колоде должно быть не более 10 специальных карт. \n";
		if (warning != "") return aviso("Предупреждение", warning);
		let me_deck = {
		...
		if (game.fullAI) {
			player_me = new Player(0, "Игрок 1", me_deck, true);
			player_op = new Player(1, "Игрок 2", this.start_op_deck, true);
		} else {
			player_me = new Player(0, "Игрок 1", me_deck, false);
			player_op = new Player(1, "Игрок 2", this.start_op_deck, true);
		}
		this.elem.classList.add("hide");
	...
	uploadDeck() {
		...
		fr.onload = e => {
			try {
				...
				aviso("Ghtleght;ltybt", "Загруженная колода отформатирована некорректно! (Проверьте фракцию)");
			}
		}
	...
	deckFromJSON(json,parse) {
		...
		if (parse) {
			try {
				...
				aviso("Предупреждение", "Загруженная колода не поддается анализу!");
				return;
			}
		} else deck = JSON.parse(JSON.stringify(json));
		let warning = "";
		if (card_dict[deck.leader].row !== "leader") warning += "'" + card_dict[deck.leader].name + "' нельзя использовать в качестве лидера\n";
		if (deck.faction != card_dict[deck.leader].deck) warning += "Leader '" + card_dict[deck.leader].name + "' не соответствует фракции колоды '" + deck.faction + "'.\n";
		let cards = deck.cards.filter(c => {
			let card = card_dict[c[0]];
			if (!card) {
				warning += "ID " + c[0] + " не соответствует карте.\n";
				return false
			}
			if (!(
				...
				warning += "'" + card.name + "' нельзя использовать в колоде фракции '" + deck.faction + "'\n";
				return false;
			}
			if (card.count < c[1]) {
				console.log(card);
				warning += "В колоде содержится " + c[1] + "/" + card.count + " доступных " + card_dict[c[0]].name + " карт\n";
				return false;
			}
			return true;
		}).map(c => ({
...
function showStats(apagarFim) {
	var tabela = "";
	var aux = ["Победы", "Ничья", "Поражение"];
	for (var i = 0; i < 3; i++) {
